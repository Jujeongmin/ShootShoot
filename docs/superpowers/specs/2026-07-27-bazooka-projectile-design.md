...
## 후속 확장 — 구조물도 폭발 범위에 포함 (2026-07-27 추가)

상자 안에 들어온 구조물도 전부 부서진다. 원숭이와 같은 규칙을 쓴다.

### 판정 기준점

원숭이 발 문제와 같은 함정이 구조물에도 있다. 그룹 원점이 시각적 중심이 아니므로, 실측한 **시각적 중심**을 쓴다.

| 구조물 | 개수 | 월드 y 범위 | 중심 y |
|---|---|---|---|
| 타워 (상자 2단 + 발판) | 2 | -1.0 ~ 1.229 | **0.11** |
| 모래주머니 참호 | 2 | -1.011 ~ 0.183 | **-0.414** |

참호 실측(`tools/measure-props.mjs`): 로컬 y -0.0112 ~ 1.1829, 높이 1.1941. `GROUND_Y = -1.0`에 놓이므로 중심 로컬 오프셋은 0.586이다.

### 참호를 파괴 가능하게 만들기

참호는 지금 총알을 막기만 하고 파괴 개념이 없다. 타워와 같은 붕괴 상태(기울기 + 낙하 + 페이드)를 갖게 하고, **부서진 참호는 `getBlockingMeshes()`에서 빠진다** — 빠지지 않으면 사라진 모래주머니가 계속 총알을 막는다.

타워 그룹은 y=0에, 참호 그룹은 y=`GROUND_Y`에 있으므로, 붕괴 낙하는 각자의 기준 높이(`baseY`)에서 빼야 한다. 한 값으로 통일하면 참호가 순간이동한다.

### 점수 — 원숭이만 준다

구조물 자체는 점수를 주지 않는다. `CONFIG.score.towerCollapseBonus`(고정 200점)를 **제거**한다.

대신 타워가 무너져 죽은 원숭이는 **일반 사격과 동일한 점수 계산**(콤보·연속 배율 포함)을 받는다. 지금은 `applyTowerCollapse`가 점수를 직접 더해서 일반 경로를 우회하는데, 이를 죽은 원숭이를 반환하는 형태로 바꿔 호출부가 `killedHits`에 넣도록 한다. 일반 총기 사격에도 똑같이 적용된다.

주의: 순수 타워 명중(원숭이를 직접 맞히지 않고 기둥만 맞힘)일 때 `resolveShot`은 `isMiss: true`를 반환하므로, `resolveKillOutcome`을 그대로 태우면 붕괴로 죽은 원숭이가 점수에서 사라진다. 그 경우 `{ isMiss: false, penetrationCount: killedHits.length, hits: killedHits }`를 직접 만든다.

### 일반 총기는 그대로

참호 파괴는 바주카 폭발로만 일어난다. 일반 총기로 참호를 쏘면 지금처럼 막히기만 한다.
