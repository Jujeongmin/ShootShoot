...
프레임 지름이 화면 높이의 절반이 된다. 조준 FOV 9°에서 화면 세로가 월드 약 12.6유닛을 덮으므로(원숭이 대열 z≈-80 기준), `R`은 월드 약 3.1유닛, 상자 전체 폭은 약 6.1유닛이다.

레인 간격은 라운드마다 다르다. `MAX_LANE_ANGLE = 0.09`이고 레인은 그 범위에 균등 배치되므로, z≈-78에서:

| 레인 수 | 라운드 (원숭이 수) | 인접 레인 간격 | 상자에 옆 레인이 들어오나 |
|---|---|---|---|
| 1 | 1 (3마리) | — | — |
| 2 | 2–4 (4–6마리) | 14.0 | 아니오 |
| 3 | 5–7 (7–9마리) | 7.0 | 아니오 (간발의 차) |
| 4 | 8+ (10마리) | 4.7 | 예 |

즉 초반 라운드에는 사실상 한 레인만 잡히고, 원숭이가 꽉 차는 후반에야 옆 레인까지 걸린다. 다만 좌우 흔들림(sway) 진폭이 z≈-78에서 약 3.0유닛이라 실제로는 경계가 더 흐릿하다. 밸런스는 `blastScreenRatio` 하나로 조절한다.
