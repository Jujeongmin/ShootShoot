...
멈춤 규칙(`partitionShotPath`) 자체는 지나간 타워를 전부 모은다 — 앞 타워를 뚫고
뒤 타워 기둥까지 닿는 경로도 규칙은 막지 않는다. 다만 **지금 씬에서는 이 일이
일어나지 않는다**: 카메라가 원점에서 움직이지 않으므로 모든 샷 경로는
`x = k·z` 꼴의 직선 하나이고, `TOWER_PLACEMENTS` 상 타워 0의 기둥은 z=-72에서
x∈[0.22, 2.78], 타워 1의 기둥은 z=-75에서 x∈[-5.78, -3.22]로 x=0을 사이에 두고
갈라져 있어 직선 하나가 둘 다 지날 수 없다. 그래서 `hitTowerIndex` 하나를
**지나간 타워 번호 목록**으로 바꾸긴 하지만, 지금 배치로는 이 목록이 원소 0개
또는 1개를 넘지 않는다 — 목록으로 두는 건 나중에 배치가 바뀔 여유를 남겨두기
위한 것이다. `applyTowerCollapse` 를 부르는 자리가 반복문이 되고, 붕괴로 죽은
