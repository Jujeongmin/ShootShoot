...
초기화 대상은 `config.js` 의 스토리지 키 전부에서 **`highScoreStorageKey` 와
`settingsStorageKey` 두 개를 뺀 것**이다: 골드, 무기, 업그레이드, 바주카, 진행도,
튜토리얼, 마지막 접속. 튜토리얼 기록도 지우므로 새 게임을 시작하면 튜토리얼을
다시 본다 — 의도한 동작이다.

마우스 민감도를 남기는 이유는 그것이 게임 진행이 아니라 조작 환경이기 때문이다.
새 게임을 한다고 손에 익은 값을 다시 맞추게 할 이유가 없다. 최고점수와 같은 취급이다.
